...
public class ResearchUpdateHandler {
	...
	public void pickupItem(PlayerTickEvent event) {
		...
		if (event.player.inventory.hasItemStack(stack)) {
		...
		if (ResearchList.start.getState().equals(EnumResearchState.FOUND)) {
			...
			if (event.player.inventory.hasItemStack(stack1)) {
			...
			if (event.player.inventory.hasItemStack(stack2)) {
			...
			if (event.player.inventory.hasItemStack(stack3)) {
